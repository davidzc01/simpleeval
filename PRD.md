...
| REQ-9 | 一项目一评测集 + case 标签筛选 | **P0** | 中 | 收敛：一个项目只有一整套测试集（UI 去除多评测集选择器）；增强：`EvalCase` 增 `tags: list[str]`（自定义标签：基准测试/回归测试/高频问题…），发起 run 时按标签多选/组合（含任一/含全部）筛选参与 case。连带：B-22 删除语义改为清空/重置评测集；批量操作支持按标签选择；REQ-7 定时回归可绑定标签（如只跑「回归测试」）；V-2 采样分析增标签维度 |
...
| 序 | 需求 | 难度 | 依赖 |
|---|---|---|---|
| 11 | REQ-8 版本概念 + 跨版本对比（与 run 对比视图合并） | 中高 | 历史数据积累 |
| 12 | REQ-7 定时回归 | 中高 | REQ-6 + 服务常驻 |
| 13 | pass@k 采样策略（执行层） | 中 | 无（历史聚合版已兑底） |
| 14 | 并发执行 | 中 | 无 |
