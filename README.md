...
- **Judge 可比性**：Judge 配置指纹记录，变更时自动提示；规则类 case 恒可比
- **评测 ROI**：模型价格配置后，每次评测自动换算成本金额——「¥0.30 拦截 2 个失败」
...
- **Judge 可比性机制**：每次 run 记录 Judge 配置指纹；Judge 变更时 delta 自动提示「仅供参考」；趋势图支持「仅规则类」切换（规则类 case 不受 Judge 影响，恒可比）
- **评测 ROI 报告**：模型价格配置（前缀匹配）→ 每次评测自动换算成本金额——「本次评测 ¥0.30 · 拦截 2 个失败」
