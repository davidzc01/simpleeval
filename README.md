...
一个面向黑盒 LLM 工作流与 Agent 的评测工具。给评测集，跑一遍，得到通过率、成本和稳定性——不用再说"观察一下"。
...
大多数评测工具假设你在自己的代码里调用模型，可以埋 SDK、采轨迹。但现实中大量 AI 应用搭在 FastGPT 这类低代码平台上——**评测对象是一个黑盒工作流**：请求进去了，结果出来了，中间看不见。

simpleEval 只依赖黑盒的输入输出：
...
它是从真实使用里长出来的：走访提取工作流的 18 条验证集、企业提取工作流的 25 条回归基线，都在这个工具上跑过。
...
> ⚠️ 评测时不要用 `--reload`：reload 监控工作目录，评测过程每 case 落盘可能触发静默重启，后台任务被杀且无报错。`--reload` 只适合纯前端/接口开发调试。
...
企业提取工作流（FastGPT 上的一条政务新闻抽取管线）有 25 条回归用例，按场景分组打标签：

- 场所载体 / 产品模型 / 品牌平台 / 载体背景 / 多主体消歧 / 基础规则 / 复合长句
...
25 条全绿才上线。改了什么、影响了哪组、稳定性有没有掉——数字说话，不用"感觉好像没问题"。
