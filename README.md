...
- **批量预估**：投产前测算 N 条任务的成本/时长区间——目标正确率下的总花费与时间
...
- **版本切换**：被测 API 版本锚点（current_version_id）——换模型/供应商时切版本，新 run 自动归属
- **pass rate 口径显性化**：所有通过率标注「基于 N/M 条」；趋势图按口径分色连线（全量主色线 + 标签组分色线，杜绝混合口径虚假波动）
- **模型价格峰谷定价**：峰价/谷价 + 峰时段，成本按时段计价；价格可直接引用 Judge/Target 配置的模型
- **批量预估**：基于历史分布预估 N 条任务的成本/时长区间（P50/P5-P95），样本分级（不足拒绝估算）
- **质量-成本闭环预估**：目标正确率 → 重跑消耗 → 总成本/总时长（含峰谷分段与谷时停产）——「多花 ¥X 把失败率从 8% 压到 0%」
- **规则类型扩充**：新增 regex / json_schema / numeric / script 四种确定性判定——免 Judge、恒可比；script 支持 if/else 与跨字段条件（评测类型从五类扩为九类）
