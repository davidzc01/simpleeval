...
**Judge 变更提示**：如果对比的两次 run 使用了不同 Judge 配置，delta 上方会出现黄条「仅供参考」——因为换 Judge 等于换刻度尺。趋势图的「仅规则类」切换只看不经过 Judge 的确定性判定（exact/contains/length 等），恒可比。

**成本与 ROI**：在配置管理页设置模型价格（¥/百万 token，前缀匹配）后，每次评测自动换算成本金额，run 详情顶部显示「本次评测 ¥0.30 · 拦截 N 个失败」。未配置价格时仅显示 token。
