...
**版本切换**：概览顶部版本条可切换当前版本——对应被测 API 的变更（换主模型/供应商）。切换后新发起的 run 自动归属该版本，概览数据以它为准。

**pass rate 口径**：所有通过率都标注「基于 N/M 条 case」——单跑冒烟组的 100% 与全量的 80% 不可直接比，口径写出来了才知道。趋势图按口径分色连线：全量主色线、高频标签组各一条色线（组内连线合法），自定义零散组为空心点。

### 批量预估（投产测算）

概览页「批量预估」：输入规模 N、目标正确率、重跑策略、时段（峰/谷）、生产端并发度 → 输出成本/时长区间（P50 期望 + P5~P95 区间）与重跑轮数——「多花 ¥X、多跑 T 分钟，把失败率从 8% 压到 0%」。

预估的几个诚实条件：数据仅取当前/指定版本；样本不足 30 拒绝估算、30~100 低置信标注；必须跨 ≥2 次 run；成本仅计被评测对象消耗（不含 Judge）；峰谷按时段计价（仅谷时模式含峰时停产等待）；生产并发与采集不一致时提示偏差。
